...
let modalOpen = (postidRef) => {
  ...
  axios.get(`/JWT-Auth/post/${postidRef}`)
    .then((postData) => {
...
let delFunc = (event) => {
  ...
  axios.delete(`/JWT-Auth/post/delete/${postID}`)
    .then((response) => {
      console.log(response)
      let postContainer = document.getElementById(`postList`)
      postContainer.innerHTML = ''
      //let successModalContainer = document.getElementsByClassName(`error-modal-container`)[0]
      // if (!successModalContainer.classList.contains(`bg-red-700`)) {
      //   successModalContainer.classList.add(`bg-red-700`)
      // }
      displayPost();
      succesOpenModal(response)
      errorModel()
...
let Update = (event) => {
  ...
  axios.put(`/JWT-Auth/post/update/${theRef}`, {
    PostTitle: postTitle.value,
    Desc: postDetails.value
  })
    .then((postData) => {
